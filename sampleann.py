...
def load_and_prepare_data():
    ...
    # Create empty lists to hold the extracted features and labels
    features = []
    labels = []
            ...
            for image_name in os.listdir(plant_folder):
                ...
                if image_name.endswith(('.jpg', '.jpeg', '.png', '.bmp', '.tiff')):
                    try:
                        ...
                        feature = base_model.predict(image).flatten()  # Flatten the features
...
def build_ann_model(input_shape, num_classes):
    """Build and compile a simple ANN model."""
    model = Sequential()
...
def train_ann_model():
    ...
    ann_model = build_ann_model(X_train.shape[1], y_train.shape[1])
    ...
    print(f'Trained ANN Model Accuracy: {accuracy * 100:.2f}%')
    messagebox.showinfo("Training Complete", f"ANN model trained with accuracy: {accuracy * 100:.2f}%")

def predict_new_image_ann(image_path):
    global ann_model
    ...
    # Extract features using VGG16
    base_model = VGG16(weights='imagenet', include_top=False, input_shape=(224, 224, 3))
    feature = base_model.predict(image).flatten().reshape(1, -1)  # Reshape to match input format

    # Predict the plant name using the ANN model
    predictions = ann_model.predict(feature)
...
def start_detection_ann():
    if not image_path:
        messagebox.showerror("Error", "Please upload an image first.")
        return

    try:
        predicted_label = predict_new_image_ann(image_path)
...
def start_training_thread_ann():
    training_thread = threading.Thread(target=train_ann_model)
...
detect_button = tk.Button(result_frame, text="Predict", command=start_detection_ann, font=("Times New Roman", 14), bg='light green', fg='black')
...
train_button = tk.Button(root, text="Train ANN Model", command=start_training_thread_ann, font=("Times New Roman", 14), bg='light green', fg='black')
